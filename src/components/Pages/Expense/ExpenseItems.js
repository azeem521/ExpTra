...
const ExpenseItems = () => {
  ...
  const getDataFrom = async () => {
    try {
      ...
      if(!response.ok){
       return alert(`status code: ${response.status}`)
      }
  ...
  const editHandlerFun = (id, amount1, catagory1, description1) => {
    setshowExp(true);
    setisEditing(true);
    setid(id);
    setAmount(amount1);
...
// };
  const toDeleteData = async (id) => {
    ...
    dispatch(expAction.deleteItemHandler(id));
...
if(isEditing){
  ...
  dispatch(expAction.editHandler(itmToAdd));
  setisEditing(false);
  ...
  dispatch(expAction.addItemHandler(itmToAdd));
}
